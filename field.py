    ...
    if fleet_bot[x][y] == 0 or fleet_bot[x][y] == 1:
        ...
        if shooted or dead:
            ...
            if end:
                ...
                # увеличение счетчика количества игр
                inc_n_game()
...
def arrange(parent_plr, parent_bot):
    if askyesno('Добро пожаловать в Классический морской бой!',
        ...
        # изменение поля статистики
        for i in range(10):
            for j in range(10):
                if (fleet_plr[i][j] == 3):
                    stat_field[i][j] = int(stat_field[i][j]) + 100
    else:
...
def clicksPlr(parent_plr, parent_bot, btn, x, y,
    ...
    global stat_field
    stat_field[x][y] = int(stat_field[x][y]) + 100
    if permit:
